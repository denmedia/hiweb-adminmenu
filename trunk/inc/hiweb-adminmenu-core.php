<?php
	...
	if( !function_exists( 'hiweb_adminmenu' ) ){
		...
		class hw_adminmenu{
			...
			/**
			 * @return hw_adminmenu_hooks
			 */
			public function hooks(){
				static $class;
				if( !$class instanceof hw_adminmenu_hooks )
					$class = new hw_adminmenu_hooks();
				return $class;
			}
		}


		class hw_adminmenu_hooks{
			...
			public function wp_ajax_hw_adminmenu_edit_item(){
				include dirname( dirname( __FILE__ ) ) . '/parts/wp_ajax_hw_adminmenu_edit_item.php';
				die;
			}


			public function wp_ajax_hw_adminmenu_save_items(){
				echo json_encode( hiweb_adminmenu()->do_save_items( $_POST['items'], $_POST['items_hide'] ) );
				die;
			}
